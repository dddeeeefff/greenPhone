...
%>
<style>
#schModel { margin: 20px 0; }
#chkBox {
	border:1px solid black;
	height:200px;
	margin: 0 0 30px 0; padding:5px;
}
.selectBox { margin:10px 0; }
.imgList { width:28px; }
.productList { 
	width:1000px; margin:50px 500px 0 400px; 
}
.imgBlack { width:60%; }
</style>
<script>
		...
		<tr align="right">
			<td colspan="2">
				...
				<form name="frm2">
					...
					<input type="text" name="pdt" placeholder="모델명을 입력하세요." value="<%=name %>" style="width:200px; height:30px; font-size:20px;" />&nbsp;&nbsp;&nbsp;
					<input type="button" value="찾기" style="width:60px; height:30px; font-size:20px;" onclick="makeSch();" />
				</form>
			</td>
		</tr>
		<tr>
			...
			%>
					<input type="radio" name="brd" id="brd<%=i %>" value="<%=pb1.getPb_id() %>" 
					<% if (chkBrd.indexOf(pb1.getPb_id()) >= 0 ) { %>checked="checked"<% } %> />
			...
			<% } %>
						...
						<input type="text" name="ep" class="pricee" placeholder="최대금액" onkeyup="onlyNum(this);" value="<%=ep %>" />
					</div>
					<p></p>
				</div>
				<p></p>
				<input type="button" value="적용하기" onclick="makeSch();" style="width:200px; height:30px; font-size:20px;" />
			</form>
			</td>
			<td width="*">
			<%
			if (rcnt > 0) {	// 검색된 상품 목록이 있을 경우
				...
				String imgList = "/greenPhone/src/img/ico_l.png";
				String imgGall = "/greenPhone/src/img/ico_g.png";
				if (v.equals("g"))	imgGall = "/greenPhone/src/img/ico_g_on.png";
				else				imgList = "/greenPhone/src/img/ico_l_on.png";
				
				String lnk = "product_list?cpage=" + cpage + sargs;
				// 정렬 및 보기 방식용 공통 링크 부분
			%>
				<p align="right" id="schModel">
					<select name="o" onchange="location.href='<%=lnk %>&v=<%=v %>&o=' + this.value" style="width:120px; height:30px; font-size:20px;">
				...
				<table width="100%" cellpadding="5" cellspacing="0" style="margin:30px;">
				<%
					if (v.equals("g")) {	// 보기 방식이 갤러리형일 경우
						...
						for (i = 0 ; i < productList.size() ; i++) {
							...
							if (pi.getStock() == 0) {
								lnk = "javascript:alert('해당 상품은 품절입니다.');";
							}
				...
				%>
					<td width="33.3%" align="center" onmouseover="this.bgColor='#efefef';" onmouseout="this.bgColor='';">
						<a href="<%=lnk %>"><img src="/greenPhone/buy/pdt_img/<%=pi.getPi_img1() %>" class="imgBlack" border="0" />
						<br /><%=brand %>
						<br /><%=pi.getPi_name() %>
						<br /><%=price %><br />
						</a>
				...
				%>
					<tr align="center" onmouseover="this.bgColor='#efefef';" onmouseout="this.bgColor='';">
						<td>
							<a href="<%=lnk %>"><img src="/greenPhone/buy/pdt_img/<%=pi.getPi_img1() %>" class="imgBlack" width="100" height="100" border="0" /></a>
						</td>
